...
function highlightAddressMatches(text, address) {
    ...
    return text.replace(regex, match => `<span class="address-highlight">${match}</span>`);
}
    ...
    try {
        ...
        if (data && Array.isArray(data) && data.length > 0) {
            ...
            const filteredData = data.map(block => {
                ...
                if (filteredBlock.extrinsics && typeof filteredBlock.extrinsics === 'object') {
                    const filteredExtrinsics = {};

                    Object.entries(filteredBlock.extrinsics).forEach(([_, extrinsics]) => {
...
window.nextBlockPage = function() {
    const totalPages = Math.ceil(window.blockData.blocks.length / window.blockData.pageSize);
